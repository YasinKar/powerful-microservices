...
async def create_category(category: CategoryCreate, db: SessionDep, current_user: CurrentUserDep):
    return await CategoryService.add_category(db, category, current_user)


@router.get("/{category_id}", response_model=Category)
async def get_category(category_id: uuid.UUID, db: SessionDep):
    return await CategoryService.get_category(db, category_id)


@router.patch("/{category_id}", response_model=Category)
async def update_category(category_id: uuid.UUID, category_update: CategoryUpdate, db: SessionDep, current_user: CurrentUserDep):
    return await CategoryService.update_category(db, category_id, category_update, current_user)


@router.delete("/{category_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_category(category_id: uuid.UUID, db: SessionDep, current_user: CurrentUserDep):

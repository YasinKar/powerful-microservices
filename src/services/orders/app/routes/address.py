...
async def get_user_addresses(current_user: CurrentUserDep):
    ...
    addresses = await AddressService.get_user_addresses(current_user.id)
    return addresses
...
async def get_user_address(address_id: uuid.UUID, current_user: CurrentUserDep):
    ...
    address = await AddressService.get_user_address(str(address_id), current_user.id)
    if not address:
...
async def update_address(address_id: uuid.UUID, address_update: UserAddress, current_user: CurrentUserDep):
    ...
    updated = await AddressService.update_address(str(address_id), current_user.id, address_update.model_dump(exclude_unset=True))
    if not updated:
...
async def delete_address(address_id: uuid.UUID, current_user: CurrentUserDep):
    """Delete an address"""
    success = await AddressService.delete_address(str(address_id), current_user.id)
    if not success:
        raise HTTPException(status_code=404, detail="Address not found or not owned by user")
